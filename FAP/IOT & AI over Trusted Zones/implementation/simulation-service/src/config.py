...
class SimulationConfig(BaseModel):
    ...
    time_acceleration: int = Field(
        default=1, ge=1, le=1000, description="Time acceleration factor"
    )
    start_time: str | None = Field(
        default=None, description="Simulation start time (ISO format)"
    )
    end_time: str | None = Field(
        default=None, description="Simulation end time (ISO format)"
    )
...
def load_config(config_path: Path | None = None, env: str | None = None) -> Settings:
    """
    ...
    if modbus_port := os.getenv("MODBUS_PORT"):
        settings_dict.setdefault("api", {}).setdefault("modbus", {})["port"] = int(
            modbus_port
        )
