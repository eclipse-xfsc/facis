...
This setup runs only the published `ecofacis/xfsc-orce:2.0.13` container. Run
the commands below from the repository root.

1. **Start it (local, no cloud):** `cd tracks/orce && docker compose pull && docker compose up -d` -> editor at <http://localhost:1880>
2. **See a working flow:** menu ≡ -> Import -> select `../../contracts/team-example-weak-flow.json` -> Import -> **Deploy** (top right).
