@UC-04-02 @FR-SM-04 @FR-SM-26
Feature: Counterparty Authorization Verification
  The system verifies counterparty PoA credentials and delegation
  chains before allowing contract signing to proceed.
  Note: This feature implements UC-04-02 from the specification.
  ...
  Scenario: Discrepancy in counterparty credentials triggers notification
    Given I am authenticated with role "Contract Signer"
    And contract "Critical Agreement" requires counterparty signature
    And counterparty "Suspicious Corp" has credential discrepancies
    When I verify counterparty authorization for "Suspicious Corp"
    Then I am notified of the discrepancies
    And the contract signing is blocked until discrepancies are resolved
